...
- **Timezone — PARTIALLY CONFIRMED.** Values come from Oracle `created_at`/`updated_at` columns rendered via Python `datetime.isoformat()`. If the DB returns tz-aware datetimes the ISO string carries an offset (parseable by `Date.parse`); if naive, the offset is absent and JS interprets it as local time. The classifier tolerates this either way (an unparseable or absent timestamp degrades to `stale`, never a false `ok`). Could **not** verify the live wire timezone offset from this machine — the gateway runs on avm-02 and was not queried.
