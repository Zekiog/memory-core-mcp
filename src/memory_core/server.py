...
class _BearerASGI:
    ...
    def __init__(self, app: Any, token: str,
                 token_map: dict[str, dict] | None = None) -> None:
        self._app = app
        self._token = token
    ...
    async def __call__(self, scope: dict, receive: Any, send: Any) -> None:
        ...
        agent_meta = None
        for cand, meta in self._map.items():
            if hmac.compare_digest(token, cand):
                agent_meta = meta
                break
        if agent_meta is None and self._token and hmac.compare_digest(token, self._token):
            agent_meta = {"agent": "legacy", "write_scopes": None}
        if agent_meta is None:
            ...
            return
        scope["memory_agent"] = agent_meta
        await self._app(scope, receive, send)


def _write_scope_allowed(request: Any, target_scope: str) -> bool:
    """Scope-prefix write check for /ingest and /catalog/upsert.

    No auth layer or legacy token (write_scopes=None) → unrestricted,
    preserving pre-MEMORY_BEARER_MAP behavior.
    """
    meta = request.scope.get("memory_agent")
    if not meta or meta.get("write_scopes") is None:
        return True
    return any(target_scope == p or target_scope.startswith(p)
               for p in meta["write_scopes"])


def _forbidden_response(request: Any, target_scope: str) -> Any:
    from starlette.responses import JSONResponse
    agent = (request.scope.get("memory_agent") or {}).get("agent", "?")
    return JSONResponse({"error": "scope_forbidden", "agent": agent,
                         "scope": target_scope}, status_code=403)
...
async def _http_ingest(request: Any) -> Any:
    ...
    target_scope = data.get("scope") or cfg.default_scope
    if not _write_scope_allowed(request, target_scope):
        return _forbidden_response(request, target_scope)
...
def _parse_bearer_map(raw: str) -> dict[str, dict]:
    """MEMORY_BEARER_MAP: optional JSON {token: {agent, write_scopes:[prefix]}}.

    Invalid JSON or shape aborts startup (fail-fast); token values are never
    echoed in the error.
    """
    if not raw:
        return {}
    try:
        ...
        return {
            str(tok): {"agent": str(meta["agent"]),
                       "write_scopes": [str(s) for s in meta["write_scopes"]]}
            for tok, meta in parsed.items()
        }
    except Exception as exc:
        raise SystemExit(
            f"FATAL: invalid MEMORY_BEARER_MAP ({type(exc).__name__})") from None
...
def main() -> None:
    ...
    token_map = _parse_bearer_map(os.getenv("MEMORY_BEARER_MAP", "").strip())
    if bearer or token_map:
        app = _BearerASGI(app, bearer, token_map)
    uvicorn.run(
        app,
        host=os.getenv("MCP_HOST", "127.0.0.1"),
        port=int(os.getenv("MCP_PORT", "8848")),
